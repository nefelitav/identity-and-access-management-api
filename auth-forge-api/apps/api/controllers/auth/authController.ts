...
export class AuthController {
    static async register(req: RegisterRequest, res: Response<RegisterResponse>): Promise<void> {
        try {
    ...
    static async login(req: LoginRequest, res: Response<LoginResponse>): Promise<void> {
        try {
            const { email, password } = req.body;
            const userAgent = req.headers['user-agent'];
            const ip = req.ip;

            const tokens = await AuthService.login({ email, password, userAgent, ip });
    ...
    static async logout(req: LogoutRequest, res: Response<LogoutResponse>): Promise<void> {
        try {
